...
fun ProjectsScreen(state: RemoteUiState, vm: RemoteViewModel, nav: NavHostController) {
    val successCount = state.projects.count { state.projectBuildStatuses[it.id] == com.antigravity.remote.ui.BuildVisualStatus.SUCCESS }
    LazyColumn(
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            HeroTag(
                                text = when (state.projectBuildStatuses[project.id]) {
                                    com.antigravity.remote.ui.BuildVisualStatus.SUCCESS -> "build saudável"
                                    com.antigravity.remote.ui.BuildVisualStatus.ERROR -> "atenção"
                                    null -> "sem status"
                                },
                                accent = projectBuildStatusColor(state.projectBuildStatuses[project.id]),
                                warm = state.projectBuildStatuses[project.id] == com.antigravity.remote.ui.BuildVisualStatus.ERROR,
                            )
                        }
                    }
                    Icon(
...
private fun projectBuildStatusColor(status: com.antigravity.remote.ui.BuildVisualStatus?): androidx.compose.ui.graphics.Color = when (status) {
    com.antigravity.remote.ui.BuildVisualStatus.SUCCESS -> StatusActive
    com.antigravity.remote.ui.BuildVisualStatus.ERROR -> StatusError
    null -> StatusIdle
}
